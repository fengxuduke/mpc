...
enum MsgType // do not use enum class here as this will cause type error when calling writeframe and coverting to unsigned int
{
    Quote = 101,
    Entrust = 102,
    Transaction = 103,
...
static inline bool is_greater(double x, double y)
{
    return std::isgreater(x - y, EPSILON);
}

static inline bool is_less(double x, double y)
{
...
static inline bool is_greater_equal(double x, double y)
{
    return is_greater(x, y) || is_equal(x, y);
}

static inline bool is_less_equal(double x, double y)
{
...
static inline bool is_too_large(double x)
{
    return is_greater(x, DOUBLEMAX);
}

static inline bool is_valid_price(double price)
{
    return !is_less_equal(price, 0.0) && !is_too_large(price);
}

static inline double rounded(double x, int n)
{
...
static inline bool string_equals(const std::string &s1, const std::string &s2)
{
    return std::strcmp(s1.c_str(), s2.c_str()) == 0;
}

static inline bool string_equals_n(const std::string &s1, const std::string &s2, size_t l)
{
...
static inline void to_upper(std::string& data)
{
...
static inline std::string to_upper_copy(const std::string& data)
{
    std::string rtn = data;
    to_upper(rtn);
    return rtn;
...
static inline void to_lower(std::string& data)
{
    std::for_each(data.begin(), data.end(), [](char & c){
        c = ::tolower(c);
    });
...
static inline std::string to_lower_copy(const std::string& data)
{
    std::string rtn = data;
    to_lower(rtn);
    return rtn;
...
static inline bool is_final_status(const OrderStatus &status)
{
    switch (status)
    {
...
static inline bool is_reverse_repurchase(const std::string &instrument_id, const std::string &exchange_id)
{
...
static inline double get_reverse_repurchase_commission_rate(const std::string &instrument_id)
{
...
static inline int get_reverse_repurchase_expire_days(const std::string &instrument_id)
{
    if (string_equals(instrument_id, "204001")|| string_equals(instrument_id, "131810"))
    {
        return 1;
...
static inline InstrumentType get_instrument_type(const std::string &instrument_id, const std::string &exchange_id)
{
...
static inline std::string str_from_instrument_type(InstrumentType type)
{
    switch(type)
    {
...
static inline std::string get_instrument_product(const char *instrument_id)
{
    std::string product = "";
    int i = 0;
...
static inline Direction get_future_direction(Side side, Offset offset)
{
...
static inline bool is_open(InstrumentType instrument_type, Side side, Offset offset)
{
...
static inline std::string get_exchange_id_from_future_instrument_id(const std::string &instrument_id)
{
...
static inline std::string get_exchange_id_from_stock_instrument_id(const std::string &instrument_id)
{
...
static inline uint32_t get_symbol_id(const std::string &symbol, const std::string &exchange)
{
    return hash_str_32(symbol) ^ hash_str_32(exchange);
}
